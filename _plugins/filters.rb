module Stackbit
  ...
  module Filters
    ...
    def get_page_by_file_path(page_path)

      site = @context.registers[:site]
      relative_page_path = replace_regexp(trim_slash(page_path), '\.md$', '')
      relative_page_parts = relative_page_path.split('/')

      if site.collections.key?(relative_page_parts[0])
        if relative_page_parts.length > 2
          raise ArgumentError, "get_page_by_file_path can not have sub-folders inside collection folder, received #{page_path}"
        end
        return site.collections[relative_page_parts[0]].docs.find do |doc|
          doc.basename == relative_page_parts[1]
        end
